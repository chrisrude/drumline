...
class AnswerSegments {
  ...
  in_answer_at_location(
    cursorLocation: GridLocation,
...
class GameState implements GameStateType {
  ...
  getAnswerSegmentsAtLocation(
    cursorLocation: GridLocation,
    use_band: boolean,
  ): AnswerSegments {
    const clue_list = this.puzzle.getClueListAtLocation(
      cursorLocation,
      use_band,
    );
    return this.getAnswerSegments(clue_list);
