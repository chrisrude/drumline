...
export const ROW_IDENTIFIERS = Array.from({ length: MAX_LENGTH }, (_, i) =>
  (i + 1).toString(),
);
...
// lower case a-z
export const CLUE_IDENTIFIERS = BAND_IDENTIFIERS.map((letter) =>
  letter.toLowerCase(),
);
...
function read_into_rowClues_and_bandClues(
  lines: string[],
): [Clue[][], Clue[][]] {
