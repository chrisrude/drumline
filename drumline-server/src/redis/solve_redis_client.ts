...
class SolveRedisClient {
    ...
    get_solve_actions = async (
        ...
        return (await this._client.lRange(list_key, start_offset, end_offset)).map(
            (str_action: string, idx: number) => {
    ...
    add_solve_action = async (solve_id: string, action: GameActions): Promise<number> => {
        ...
        return await this._client.rPush(list_key, str_action);
    };

    _solve_id_key = (solve_id: string): string => `solve:${solve_id}`;

    _action_list_key = (solve_id: string): string =>
        `${this._solve_id_key(solve_id)}:${ACTION_LIST_KEY_SUFFIX}`;
}
