...
export {
    ...
    memoizedGetBandNumberFromCoords,
    memoizedLocationsForBand,
    memoizedLocationsForRow,
    nextIndexWithWrap,
    nextIndexWithoutWrap,
    prevIndexWithWrap,
    prevIndexWithoutWrap
};
export type {
    BandGroup,
    BandSides,
...
const _clamp = (val: number, min: number, max: number) => Math.min(Math.max(val, min), max);

// returns the index'ed value of the list, clamping to the
// first or last element if an out of bounds index is given
const _clampListIndex = <T>(list: T[], idx: number): number => _clamp(idx, 0, list.length - 1);
...
const _generateGridAttributes = (size: number): GridAttributes => {
    ...
    const locations_for_band = Array.from({ length: num_bands }, (_, i) =>
        memoizedLocationsForBand(size, i)
    );
    const locations_for_row = Array.from({ length: size }, (_, i) =>
        memoizedLocationsForRow(size, i)
    );
