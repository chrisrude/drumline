...
class PuzzleRedisClient {
    ...
    loadPuzzle = async (puzzle_id: string): Promise<Puzzle | null> => {
        const results = await this._client.hGet(this._puzzleKey(puzzle_id), PUZZLE_INPUT_SUBKEY);
        if (!results) {
            // puzzle not found
            return null;
        }
    ...
    _get_puzzle_metadata = async (puzzle_id: string): Promise<[string, number]> => {
        const results = await this._client.hmGet(this._puzzleKey(puzzle_id), [
    ...
    _puzzleKey = (puzzle_id: string): string => `${PUZZLE_KEY_PREFIX}:${puzzle_id}`;
